...
#define LED_OFF 1
#define LED_ON 0
int current_state =LED_ON;
bool notify_enabled = false;
void led_flash_init(void)
{
    gpio_config_t led_gpio_cfg = {
        .pin_bit_mask = (1ULL << LED_GPIO),          //指定GPIO
...
static esp_attr_value_t gatts_demo_led_state_val =
{
...
/*
    字段	            类型	                说明
    gatts_cb	        esp_gatts_cb_t	        GATT 事件回调函数，处理该 Profile 的 BLE 事件（如连接、读写请求）。
    gatts_if	        uint16_t	            GATT 接口句柄，由 ESP_GATTS_REG_EVT 事件返回，用于标识不同的 Profile。
    app_id	            uint16_t	            应用程序 ID，用于区分不同的 BLE 应用（通常与 PROFILE_X_APP_ID 对应）。
    conn_id	            uint16_t	            当前连接 ID，记录哪个客户端设备已连接。
    service_handle	    uint16_t	            服务句柄，唯一标识一个 GATT 服务。
    service_id	        esp_gatt_srvc_id_t	    服务 UUID 和实例 ID，定义服务的类型（如 0x1801 为通用属性服务）。
    char_handle	        uint16_t	            特征值（Characteristic）句柄。
    char_uuid	        esp_bt_uuid_t	        特征值的 UUID（如 0x2A37 为心率测量）。
    perm	            esp_gatt_perm_t	        特征值的权限（如 ESP_GATT_PERM_READ 表示可读）。
    property	        esp_gatt_char_prop_t	特征值的属性（如 ESP_GATT_CHAR_PROP_BIT_NOTIFY 支持通知）。
    descr_handle	    uint16_t	            描述符（Descriptor）句柄（如 CCCD 用于启用通知）。
    descr_uuid	        esp_bt_uuid_t	        描述符的 UUID（如 0x2902 为 CCCD）。
...
void light_state_changed(uint8_t new_state)
{
    current_state = new_state;
    const char *state_str = (current_state == LED_ON) ? "LED_ON" : "LED_OFF";
    uint16_t len = strlen(state_str) + 1; // 包括null终止符
    
    uint8_t *led_state = (uint8_t *)malloc(len);
    strcpy((char *)led_state, state_str);
    
    esp_ble_gatts_set_attr_value(GATTS_CHAR_UUID_TEST_A,
                                len,
                                led_state);
    ...
    /* 若客户端已打开通知，则立即发送 */
    if (notify_enabled) {
        esp_ble_gatts_send_indicate(gl_profile_tab[0].gatts_if,
                                    gl_profile_tab[0].conn_id,                                   // 当前连接
                                    gl_profile_tab[0].char_handle,    // 特征值 handle
                                    sizeof(uint8_t)*len,
                                    led_state,
                                    false); // true = indication(带ACK)；false = notification
    }
    free(led_state);
}
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        int led_state = gpio_get_level(GPIO_NUM_35);
        if(led_state)
        {
            mempcpy(rsp.attr_value.value,"LED_OFF",7);
            rsp.attr_value.len = 7;
        }
        else
        {
            mempcpy(rsp.attr_value.value,"LED_ON",6);
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            if (gl_profile_tab[PROFILE_A_APP_ID].descr_handle == param->write.handle && param->write.len == 2){
                ...
                ESP_LOGI(GATTS_TAG,"客户端写入：%s\n",param->write.value);
                if(!strcmp((const char *)param->write.value,"LED_OFF"))
                {
                    ESP_LOGI(GATTS_TAG,"客户端请求关灯\n");
                    gpio_set_level(GPIO_NUM_35,1);
                    light_state_changed(1);
                }
                else if(!strcmp((const char *)param->write.value,"LED_ON"))
                {
                    ESP_LOGI(GATTS_TAG,"客户端请求开灯\n");
