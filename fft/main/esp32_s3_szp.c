...
static i2c_master_dev_handle_t s_i2c_bus_handle = NULL;
...
void qmi8658_init(void)
{
    ...
    ESP_ERROR_CHECK(i2c_master_bus_add_device(bus_handle, &dev_cfg, &qmi8658_handle));
...
esp_err_t pca9557_register_write_byte(uint8_t reg_addr, uint8_t data)
{
    ...
    uint8_t write_buf[2] = {reg_addr, data};
    return i2c_master_transmit(pca9557_handle,write_buf, sizeof(write_buf),pdMS_TO_TICKS(1000));
}
void pca9557_debug()
{
    uint8_t config, output;
    pca9557_register_read(PCA9557_CONFIGURATION_PORT, &config, 1);
    pca9557_register_read(PCA9557_OUTPUT_PORT, &output, 1);
    ESP_LOGI("DEBUG", "PCA9557 CONFIG: 0x%02X, OUTPUT: 0x%02X", config, output);
}
// 初始化PCA9557 IO扩展芯片
void pca9557_init(void)
{
    ...
    esp_err_t err = pca9557_register_write_byte(PCA9557_OUTPUT_PORT, 0x05);
    ESP_LOGI(TAG, "PCA9557 write OUTPUT_PORT result: %s", esp_err_to_name(err)); 

    // 把PCA9557芯片的IO1 IO1 IO2设置为输出 其它引脚保持默认的输入
    ret = pca9557_register_write_byte(PCA9557_CONFIGURATION_PORT, 0xf8); // 设置P0~P2为输出
    ESP_LOGI(TAG, "Write PCA9557 CONFIG_PORT ret: %s", esp_err_to_name(ret));

    uint8_t data;
    ret = pca9557_register_read(PCA9557_OUTPUT_PORT, &data, 1);
    ESP_LOGI(TAG, "OUTPUT_PORT readback: 0x%02X, ret: %s", data, esp_err_to_name(ret));
}
...
esp_err_t pca9557_set_output_state(uint8_t gpio_bit, uint8_t level)
{
    ...
    // 先读取当前输出端口状态
    data[0] = PCA9557_OUTPUT_PORT;
    ret = i2c_master_transmit_receive(pca9557_handle, 
                                    &data[0], 1, 
                                    &data[1], 1, 
                                    pdMS_TO_TICKS(1000));
    if (ret != ESP_OK) {
    ...
    data[1] = SET_BITS(data[1], gpio_bit, level);
    // 写回新值
    return i2c_master_transmit(pca9557_handle, data, sizeof(data), pdMS_TO_TICKS(1000));
}
...
void lcd_cs(uint8_t level)
{
    ...
    ESP_LOGI(TAG, "lcd_cs: ret: %s", esp_err_to_name(ret));
    //pca9557_register_write_byte(PCA9557_OUTPUT_PORT, 0x04);
}
...
void pa_en(uint8_t level)
{
    ...
    ESP_LOGI(TAG, "pa_en: ret: %s", esp_err_to_name(ret));
}
...
void dvp_pwdn(uint8_t level)
{
    ...
    ESP_LOGI(TAG, "dvp_pwdn: ret: %s", esp_err_to_name(ret));
}
...
esp_err_t bsp_touch_new(esp_lcd_touch_handle_t *ret_touch)
{
    ...
    ESP_ERROR_CHECK(i2c_master_bus_add_device(bus_handle, &dev_cfg, &s_i2c_bus_handle));  
...
/***********************************************************/
/*********************    音频 ↓   *************************/
esp_codec_dev_handle_t play_dev_handle;    // speaker句柄
esp_codec_dev_handle_t record_dev_handle;  // microphone句柄
