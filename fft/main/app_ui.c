...
extern QueueHandle_t fft_result_queue_mp3;
extern SemaphoreHandle_t fft_data_ready;
extern int16_t *fft_pcm_buffer;
...
static esp_err_t _audio_player_write_fn(void *audio_buffer, size_t len, size_t *bytes_written, uint32_t timeout_ms)
{
    ...
    // }
    // else {
    //     ESP_LOGE(TAG, "audio_buffer size is too small, len = %d", len);
    //     memcpy(fft_pcm_buffer, audio_buffer, len);
    //     xSemaphoreGive(fft_data_ready);  // 通知 FFT 任务数据已准备
    // }
...
{
    esp_err_t ret = ESP_OK;
    ESP_LOGI(TAG, "rate = %lu, bits_cfg = %lu, ch = %d", rate, bits_cfg, ch);
